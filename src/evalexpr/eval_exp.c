...
int eval_rpn(char *expr, int rpn)
{
    ...
    while (expr[i] != '\0')
    {
        ...
        if (isdigit(expr[i]))
        {
            struct token t = { NB, mini_atoi(expr, &i) };
        ...
        else if (is_operator(expr + i))
        {
            ...
            struct token nb1 = nb_stack->data;
            nb_stack = stack_pop(nb_stack);
            if (nb_stack == NULL)
                errx(2, "Invalid expression");
            struct token nb2 = nb_stack->data;
            nb_stack = stack_pop(nb_stack);
            struct token res = { NB, compute(nb2, nb1, operator) };
            free(operator);
    ...
    struct token res = nb_stack->data;
    nb_stack = stack_pop(nb_stack);
    if (nb_stack != NULL)
        errx(2, "Invalid expression");
    printf("%d\n", res.data);
...
struct stack *append_op(struct stack *st, struct token cur, char *res,
                        size_t *j)
{
    if (cur.type == POPEN)
        st = stack_add(st, cur);
...
char *to_polish(char *expr)
{
    ...
    while (expr[i] != '\0')
    {
        ...
        {
            struct token cur = token_op(expr + i);
